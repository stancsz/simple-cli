...
describe("git tool", () => {
  ...
  beforeEach(async () => {
    testDir = join(
      process.cwd(),
      ".test_tmp",
      `simple-cli-git-test-${Date.now()}-${Math.random()}`,
    );
  ...
  describe("git log", () => {
    it("should show commit history", async () => {
      ...
      await writeFile(join(testDir, "file.txt"), "content");
      execSync('git add file.txt && git commit -m "test commit"', {
        cwd: testDir,
        stdio: "pipe",
      });
  ...
  describe("helper functions", () => {
    it("getCurrentBranch should return branch name", async () => {
      ...
      execSync('git add init.txt && git commit -m "init"', {
        cwd: testDir,
        stdio: "pipe",
      });
    ...
    it("getTrackedFiles should return tracked files", async () => {
      ...
      await writeFile(join(testDir, "file.txt"), "content");
      execSync('git add file.txt && git commit -m "add"', {
        cwd: testDir,
        stdio: "pipe",
      });
