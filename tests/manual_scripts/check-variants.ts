...
async function testProvider(name: string, provider: string, model: string) {
    ...
    try {
        const response = await llm.generate('sys', [{ role: 'user', content: 'Say HELLO' }]);
...
async function run() {
    ...
    await testProvider('Anthropic Latest', 'anthropic', 'claude-3-5-sonnet-latest');
    await testProvider('Anthropic Date-based', 'anthropic', 'claude-3-5-sonnet-20241022');
