...
async function main() {
  ...
  spawn("git", ["config", "user.email", "you@example.com"], {
    cwd: testDir,
    stdio: "ignore",
  });
  spawn("git", ["config", "user.name", "Your Name"], {
    cwd: testDir,
    stdio: "ignore",
  });
  ...
  const scriptPath = (agentFile: string) =>
    join(cwd(), "src", "agents", agentFile);
  ...
  const verifyFile = (filename: string, expectedContent: string) => {
    ...
    if (existsSync(filePath)) {
      ...
      if (content.includes(expectedContent)) {
        ...
        console.error(
          `[FAIL] ${filename} created but content mismatch. Found: "${content}"`,
        );
      }
    } else {
