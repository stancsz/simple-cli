...
process.on('uncaughtException', (err) => {
	appendLog('uncaughtException: ' + (err && (err as Error).stack ? (err as Error).stack : String(err)));
});

process.on('unhandledRejection', (reason) => {
	appendLog('unhandledRejection: ' + (reason && (reason as any).stack ? (reason as any).stack : String(reason)));
});

// Capture worker/process exit events if tinypool emits them via process 'error' events
process.on('warning', (warning) => {
	appendLog('warning: ' + (warning && (warning as any).stack ? (warning as any).stack : String(warning)));
});

process.on('error', (err) => {
	appendLog('process.error: ' + (err && (err as Error).stack ? (err as Error).stack : String(err)));
});

process.on('uncaughtExceptionMonitor', (err) => {
	appendLog('uncaughtExceptionMonitor: ' + (err && (err as Error).stack ? (err as Error).stack : String(err)));
});

process.on('message', (msg) => {
	try { appendLog('process.message: ' + JSON.stringify(msg)); } catch { appendLog('process.message: <unserializable>'); }
});
...
try {
	...
	ChildProcess.prototype.emit = function (event: string, ...args: any[]) {
		try {
			if (event === 'error') {
				try { appendLog('ChildProcess.emit error: ' + (args[0] && args[0].stack ? args[0].stack : String(args[0]))); } catch { }
	...
	try {
		...
		child_process.fork = function (...args: any[]) {
			...
			try {
				appendLog(`child.fork: pid=${cp.pid} argv=${JSON.stringify(args[1]||[])}`);
				cp.on('exit', (code: number | null, signal: string | null) => {
					appendLog(`child.exit: pid=${cp.pid} code=${code} signal=${signal}`);
				});
				cp.on('error', (err: any) => {
					appendLog(`child.error: pid=${cp.pid} err=${err && err.stack ? err.stack : String(err)}`);
				});
			} catch (e) { /* best effort */ }
			return cp;
		};
	...
	try {
		...
		child_process.spawn = function (...args: any[]) {
			...
			try {
				appendLog(`child.spawn: pid=${cp.pid} cmd=${JSON.stringify(args[0])} argv=${JSON.stringify(args[1]||[])}`);
				cp.on('exit', (code: number | null, signal: string | null) => {
					appendLog(`child.exit: pid=${cp.pid} code=${code} signal=${signal}`);
				});
				cp.on('error', (err: any) => {
...
try {
	...
	process.stderr.write = (chunk: any, ...args: any[]) => {
		try {
			const s = typeof chunk === 'string' ? chunk : chunk?.toString?.() ?? String(chunk);
	...
	process.on('exit', () => {
		try { if (buffer.length) appendLog('\n=== Recent STDERR ===\n' + buffer + '\n=== End STDERR ===\n'); } catch { }
	});
} catch (e) {
	/* ignore */
}
...
try {
	...
	const snapshot = (() => {
		try { return execSync(process.platform === 'win32' ? 'tasklist' : 'ps -ef', { encoding: 'utf-8', timeout: 5000 }); } catch (e) { return String(e); }
	})();
	appendLog('\n=== Process Snapshot ===\n' + snapshot + '\n=== End Snapshot ===\n');
} catch (e) {
	/* best effort */
