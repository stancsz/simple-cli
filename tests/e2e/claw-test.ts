...
async function runTests() {
    ...
    const inspect = await tool.execute({ action: 'inspect', skillName: 'clawJit' });
    const inspectJson = JSON.parse(inspect);
    assert.strictEqual(inspectJson.definition.name, 'clawJit', 'Should return correct skill definition');
    ...
    await tool.execute({ action: 'run', skillName: 'clawBrain', args: { action: 'init' } });
    const memoryDir = join(process.cwd(), '.simple', 'workdir', 'memory');
    assert.ok(existsSync(join(memoryDir, 'notes')), 'Memory notes dir should exist');
