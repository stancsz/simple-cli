...
vi.mock('child_process', () => ({
    ...
    execFile: vi.fn((file, args, cb) => {
        ...
        cb(null, { stdout: 'https://github.com/owner/repo/pull/1', stderr: '' }, '');
    })
}));
...
describe('pr_create', () => {
    beforeEach(() => {
        ...
        (child_process.execFile as any).mockImplementation((file: string, args: any, cb: any) => {
             if (typeof args === 'function') {
                cb = args;
                args = [];
            }
    ...
    it('should handle errors', async () => {
        (child_process.execFile as any).mockImplementation((file: string, args: string[], cb: Function) => {
