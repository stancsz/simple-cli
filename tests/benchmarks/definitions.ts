...
export const benchmarks: Benchmark[] = [
  {
    name: "Terminal-Bench",
    description:
      "Evaluates proficiency with shell commands and file system operations.",
    tasks: [
      {
        name: "List Files",
  ...
  },
  {
    ...
    description:
      "Simulates corporate tasks involving document creation and processing.",
    tasks: [
      {
        name: "Write Memo",
        prompt:
          'Write a memo to "staff.txt" about the new coffee policy: "Coffee is now free".',
        verify: (cwd) => {
  ...
  },
  {
    name: "SWE-bench",
    description:
      "Tests software engineering capabilities like debugging and refactoring.",
    tasks: [
      {
        name: "Fix Bug",
        prompt:
          "The function `add` in `math.js` is incorrect. It subtracts instead of adding. Fix it.",
        setup: (cwd) => {
          writeFileSync(
            join(cwd, "math.js"),
    ...
    tasks: [
      {
        ...
        prompt:
          "If I have 5 apples and give 2 to my friend, how many do I have left? Reply with just the number.",
        verify: (cwd, output) => {
  ...
  },
  {
    name: "OSWorld",
    description:
      "Tests ability to control the operating system environment (simulated).",
    tasks: [
      {
        name: "Move File",
        ...
        verify: (cwd) => {
          return (
            existsSync(join(cwd, "dest", "document.txt")) &&
            !existsSync(join(cwd, "source", "document.txt"))
          );
        },
      },
    ],
