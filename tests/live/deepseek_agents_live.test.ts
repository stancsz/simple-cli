...
describe('DeepSeek Agents Live Test', () => {
    ...
    beforeAll(() => {
        ...
        spawn('git', ['config', 'user.email', 'test@example.com'], { cwd: testDir, stdio: 'ignore' });
        spawn('git', ['config', 'user.name', 'Test User'], { cwd: testDir, stdio: 'ignore' });
    });
    ...
    const runAgent = async (agentName: string, args: string[]) => {
        return new Promise<void>((resolve, reject) => {
            const child = spawn('npx', ['tsx', join(process.cwd(), 'src', 'agents', agentName), ...args], {
                cwd: testDir,
    ...
    it.runIf(shouldRun)('deepseek_aider should create a file', async () => {
    ...
    it.runIf(shouldRun)('deepseek_claude should create a file', async () => {
    ...
    it.skipIf(true)('deepseek_opencode should create a file', async () => {
