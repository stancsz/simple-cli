...
describe('Scheduler Lifecycle (Live)', () => {
    ...
    it('should automatically forget a task after 3 consecutive failures', async () => {
        ...
        const id = await scheduler.scheduleTask('* * * * *', 'echo fail', 'Failing Task');
    ...
    it('should reset failure count on success', async () => {
        const scheduler = Scheduler.getInstance(TEST_DIR);
        const id = await scheduler.scheduleTask('* * * * *', 'echo success', 'Good Task');
        ...
        let content = JSON.parse(await readFile(join(TEST_DIR, '.agent', 'scheduler.json'), 'utf-8'));
        ...
        content = JSON.parse(await readFile(join(TEST_DIR, '.agent', 'scheduler.json'), 'utf-8'));
