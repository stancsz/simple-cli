...
describe("OpenClaw Integration Stress Test", () => {
  ...
  it(
    "should inspect a specific skill (weather)",
    async () => {
  ...
  it(
    "should run a simple agent task using openai-agent",
    async () => {
      ...
      try {
        ...
        console.warn(
          `Skipping agent test: Agent '${TEST_AGENT_ID}' not found. Run 'npx openclaw agents add ${TEST_AGENT_ID} --model openai/gpt-4o-mini --workspace /tmp/openclaw-openai-workspace --non-interactive' to enable.`,
        );
        return;
      }
      ...
      console.log(
        `Starting agent task with session ID: ${sessionId} using agent: ${TEST_AGENT_ID}`,
      );
