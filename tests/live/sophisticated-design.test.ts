...
const LOG_FILE = join(
  process.cwd(),
  "docs/feedbacks/sophisticated-design.log.md",
);
...
describe("Sophisticated Design Test - Runtime Tool Creation", () => {
  ...
  it(
    "should create a custom tool at runtime and use it",
    async () => {
      const prompt = `
...
`;
      ...
      expect(existsSync(resultPath), "Result file should be created").toBe(
        true,
      );
