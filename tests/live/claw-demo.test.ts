...
describe("Claw Mode Demo - File Organization", () => {
  beforeAll(() => {
    try {
      ...
      writeFileSync(
        join(DEMO_DIR, "receipt_starbucks.txt"),
        "Receipt\nTotal: $12.50\nDate: 2026-01-31",
      );
      writeFileSync(
        join(DEMO_DIR, "Expenses.csv"),
  ...
  it(
    ...
    async () => {
      ...
      writeFileSync(
        join(DEMO_DIR, "receipt_dinner.txt"),
        "Dinner Receipt\nTotal: $45.00\nDate: 2026-01-31",
      );
      ...
      const expensesContent = readFileSync(
        join(DEMO_DIR, "Expenses.csv"),
        "utf-8",
      );
      console.log("\n💰 Final Expenses Log:\n", expensesContent);
