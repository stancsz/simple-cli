...
describe('delegate_cli', () => {
    ...
    it('should use configured agent', async () => {
        ...
        const result = await delegate_cli.execute({ cli: 'real-agent', task: 'do real work' });

        expect(spawn).toHaveBeenCalledWith('real-cmd', ['arg1', 'do real work'], expect.anything());
    ...
    it('should support stdin context injection', async () => {
        ...
        const result = await delegate_cli.execute({ cli: 'stdin-agent', task: 'pipe work' });
