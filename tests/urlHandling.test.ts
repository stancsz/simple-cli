...
describe('urlHandling', () => {
  ...
  describe('URL detection', () => {
    ...
    it('should detect URL with path and query', () => {
      ...
      const matches = text.match(URL_REGEX);
      expect(matches).toContain('https://example.com/path?query=value&other=123');
    });
  ...
  describe('URL extraction from messages', () => {
    ...
    it('should extract multiple URLs from message', () => {
      const message = 'Compare http://old.example.com with https://new.example.com';
      const urls = extractUrls(message);
    ...
    it('should handle URL with complex query parameters', () => {
      const message = 'Check https://api.example.com/v1/search?q=test&limit=10&offset=0';
      const urls = extractUrls(message);
