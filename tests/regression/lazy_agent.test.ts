    ...
    async generate(prompt: string, history: any[], signal?: AbortSignal) {
        ...
        if (this.callCount === 1) {
            return {
                ...
                tool: 'none',
                args: {},
                message: "I did not create the file physically. I provided the content in the previous message.",
                raw: "..."
            };
        }
...
describe('Engine Lazy Agent Handling', () => {
    ...
    beforeEach(() => {
        ...
        registry.tools.set('write_files', {
            ...
            execute: async (args: any) => { return "Success"; }
        } as any);
    ...
    it('should retry when agent explicitly refuses (did not create)', async () => {
        ...
        await engine.run(ctx, "Create a file named test.txt", { interactive: false });
