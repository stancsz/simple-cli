...
describe("Production Deployment Integration Test", () => {
    ...
    it("should execute scheduled tasks in Ghost Mode (via mcp.json)", async () => {
        const scheduler = new Scheduler(testRoot);
        ...
        // Write to mcp.json in CWD (testRoot)
        await writeFile(join(testRoot, "mcp.json"), JSON.stringify({ scheduledTasks: [task] }));

        await scheduler.start();

        // Advance time to trigger cron
        await vi.advanceTimersByTimeAsync(1000 * 65);

        // Switch to real timers to wait for child process IO
        vi.useRealTimers();
        await new Promise(resolve => setTimeout(resolve, 2000));

        await scheduler.stop();

        // Verify execution via log file creation
        const logsDir = join(testRoot, "ghost_logs");
        try {
            const files = await readdir(logsDir);
            const logFile = files.find(f => f.includes("ghost-task-1"));
            expect(logFile).toBeDefined();

            if (logFile) {
                const content = await readFile(join(logsDir, logFile), 'utf-8');
                const logData = JSON.parse(content);
                expect(logData.taskId).toBe("ghost-task-1");
                expect(logData.taskName).toBe("Ghost Task A");
            }
        } catch (e) {
            throw new Error(`Ghost logs verification failed: ${e}`);
        }
    });
