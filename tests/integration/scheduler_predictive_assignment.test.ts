...
describe('Phase 35: Scheduler Predictive Task Assignment Validation', () => {
    ...
    it('should handle edge cases where no patterns or statuses are available', async () => {
        const mockGenerate = vi.fn().mockResolvedValue({
            raw: JSON.stringify({
                recommended_agency_id: "spawn_new",
                confidence_score: 0.5,
                reasoning: "No patterns or statuses available to make an informed decision."
            }),
            thought: "",
            tool: "none",
            args: {},
        ...
        // Mock the client to return empty data for this specific test
        vi.spyOn(console, 'warn').mockImplementation(() => {});

        const result = await assignTool.handler({
            task_description: "Do something completely novel.",
            priority: "low"
        }, {});

        expect(result.isError).toBeUndefined();
        const parsed = JSON.parse(result.content[0].text);
        expect(parsed.recommended_agency_id).toBe("spawn_new");
    });
