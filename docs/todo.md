...
- [x] **LLM Call Caching**: Implemented a caching layer (`src/llm/cache.ts`) using file-based/Redis backend with metrics integrated into Health Monitor.
- [x] **Batch Prompt Consolidation**: Bundled routine daily/hourly strategic scans into single context windows via `BatchExecutor`.
- [x] **Adaptive Model Routing**: Dynamically routes LLM requests based on task complexity scores generated by the `business_ops` MCP to optimize token costs.
