...
- [x] **Episodic Memory (Vector DB)**:
    ...
    - [x] **Brain production hardening**: Implemented `LanceConnector` and `SemanticGraph` with robust cross-process concurrency locking (`proper-lockfile`). Validated via multi-tenant integration tests.
