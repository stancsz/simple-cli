...
- [x] **Batch Prompt Consolidation**: Bundle routine daily or hourly strategic scans into single context windows to maximize contextual token usage.
    - ✅ Completed: Implemented `PromptBatcher` service in `src/llm/batching.ts` with queueing and timer logic.
    - ✅ Completed: Integrated into the Daemon Scheduler (`src/scheduler/trigger.ts`), allowing configurable `batchingGroup` IDs in `mcp.json`.
    - ✅ Completed: Integrated `batched_prompts_total` and `tokens_saved_via_batching` into the Health Monitor MCP.
    - ✅ Validated: End-to-end tests via `tests/integration/batch_prompt_consolidation.test.ts` verify multi-task prompt consolidation and token savings.
