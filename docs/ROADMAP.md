...
- [ ] **LLM Call Caching**: Implement a caching layer (`src/llm/cache.ts`) using a file-based or Redis backend to intercept identical LLM prompts and serve cached responses, thus avoiding redundant API calls. Integrate metrics (`llm_cache_hit`, `llm_cache_miss`) into the Health Monitor.
    - 🚧 In Progress: Architecture defined in `docs/LLM_CACHING.md`, cache implementation built, and metrics integrated. Pending validation.
- [ ] **Batch Prompt Consolidation**: Bundle routine daily or hourly strategic scans into single context windows to maximize contextual token usage.
