...
*Status: Completed*
*Current Focus: Phase 28 is fully operational. Caching, Batching, and Adaptive Routing have drastically reduced token consumption.*
...
- [x] **Adaptive Model Routing**: Dynamically route LLM requests (e.g., using Haiku/Flash for parsing and Opus/Reasoner for complex reasoning) based on task complexity scores generated by the `business_ops` MCP.
    - ✅ Implemented `ModelRouter` in `src/llm/router.ts` using `score_task_complexity` tool.
    - ✅ Added dynamic tier-based routing config with in-memory hashing for performance.
    - ✅ Integrated `llm_model_routing_hits` and `llm_cost_savings_estimated` metrics into Health Monitor.
    - ✅ Validated cost savings vs baseline in `tests/integration/adaptive_model_routing.test.ts`.
