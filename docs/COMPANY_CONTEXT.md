...
4.  **Brain Integration**: The `ContextManager` now automatically integrates with the Brain MCP. When loading context for a specific company, it queries the Brain for relevant past experiences within that company's scope. Task outcomes are stored in the Brain's company-specific episodic and semantic memory, ensuring complete data isolation and cross-session learning.
