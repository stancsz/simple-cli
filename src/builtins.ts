...
export const writeFiles = {
  name: "write_files",
  description:
    "Write or modify files. Use SEARCH/REPLACE blocks for partial edits.",
  ...
  execute: async (args: any) => {
    ...
    if (!files || !Array.isArray(files)) {
      throw new Error(
        `Invalid arguments for write_files: ${JSON.stringify(args)}`,
      );
    }
      ...
      try {
        ...
        if (!isPathAllowed(f.path)) {
          results.push({
            path: f.path,
            success: false,
            message: "Access denied: Path is outside the allowed workspace.",
          });
          continue;
        }
        ...
        if (f.content !== undefined) {
          ...
          for (const { search, replace } of f.searchReplace) {
            if (!content.includes(search)) {
              throw new Error(
                `Search pattern not found in ${f.path}: "${search}"`,
              );
            }
  ...
  execute: async ({
    ...
    if (existsSync(targetDir)) {
      ...
      const similar = existing.find(
        (f) => f.startsWith(name) || name.startsWith(f.split(".")[0]),
      );
      if (similar) {
...
export const runCommand = {
  name: "run_command",
  description:
    "Run a shell command. Use background: true for servers or long-running tasks.",
  inputSchema: z.object({
    command: z.string(),
    timeout: z.number().optional(),
    background: z
      .boolean()
      .default(false)
      .describe("Run command in background and return immediately"),
  }),
  execute: async (
    {
      command,
      timeout,
      background,
    }: { command: string; timeout?: number; background: boolean },
  ...
  execute: async ({
    ...
    if (operation === "checkout") {
      if (!branch_name)
        return {
          success: false,
          error: "No branch_name specified for checkout",
        };
  ...
  execute: async ({ path }: { path: string }) => {
    ...
    if (ext === ".js" || ext === ".ts") cmd = `node --check "${path}"`;
      ...
      return {
        ...
        errors: [],
        warnings: [],
        output: "No linter for this file type",
      };
    ...
    try {
      ...
      const lineMatch =
        stderr.match(/line\s+(\d+)/i) || stderr.match(/:(\d+):/);
      const line = lineMatch ? parseInt(lineMatch[1]) : 0;
      return {
        passed: false,
        language,
        errors: [{ message: stderr, line }],
        warnings: [],
        output: stderr,
        file: path,
      };
...
export const delegate_cli = {
  name: "delegate_cli",
  description:
    "Delegate a complex task to a specialized external CLI agent (Codex, Gemini, Claude).",
  inputSchema: z.object({
    cli: z.string(),
    task: z.string(),
  ...
  execute: async ({
    ...
    try {
      ...
      if (!config.agents || !config.agents[cli]) {
        console.warn(
          `[delegate_cli] No configuration found for '${cli}'. Falling back to mock.`,
        );
...
export const pr_create = {
  ...
  inputSchema: z.object({
    ...
    base: z
      .string()
      .optional()
      .describe("The branch you want your changes pulled into"),
    head: z
      .string()
      .optional()
      .describe("The branch that contains changes for your pull request"),
  }),
  execute: async ({
    title,
    body,
...
export const pr_comment = {
  name: "pr_comment",
  description:
    "Add a comment to a PR (e.g., to instruct Jules to fix something).",
  inputSchema: z.object({
    pr_number: z.number(),
    body: z.string(),
  }),
