  ...
  constructor(
    ...
    this.s = {
      start: (msg: string) => {
        ...
        // Note: Slack Web API does not support a dedicated 'typing' event easily without RTM.
        // We avoid sending text messages here to prevent spam.
      },
...
app.event("app_mention", async ({ event, say, client }: any) => {
  ...
  try {
    ...
    // Retrieve Response (Check context history, but if run() was skipped, history might be empty?)
    // If run() returns early due to working hours, history won't have the "assistant" message generated by LLM.
    // However, engine.run() in SlackEngine sends the offline message directly.
    // So we just check if we have a last message. If not, and we are offline, we did our job.

    // Check if we need to send a final summary if not offline.
    // If offline, run() returned early.
    // We can check if engine ran? No easy way.
    // But if we returned early, lastMessage might be undefined or just user message.

    // We can rely on engine.run() (super.run) to populate history.
    // If super.run wasn't called, history only has user message.
    ...
    if (lastMessage) {
      ...
      // If we are offline, we already sent a message in run().
      // If we are online but no response, then send error.
      // How to distinguish?
      // Check config again? Or just ignore if history is empty?
      // If history has no assistant message, it means LLM didn't run or didn't reply.
      // If we are offline, we don't want to send "I couldn't generate a response."
      // So we should check.

      await provider.personaEngine.loadConfig(); // Ensure loaded
