...
export class SOPExecutor {
  ...
  private async logStep(sopName: string, stepNumber: number, status: 'success' | 'failure', details: string, company?: string) {
    let currentLogDir = this.logDir;
    if (company) {
        currentLogDir = join(process.cwd(), '.agent', 'companies', company, 'brain');
    }

    if (!existsSync(currentLogDir)) {
      await mkdir(currentLogDir, { recursive: true });
    }
    const logFile = join(currentLogDir, 'sop_logs.json');
    let logs: any[] = [];
  ...
  async execute(sop: SOP, input: string, company?: string): Promise<string> {
    ...
    try {
        ...
        if (brainQuery) {
            const result = await brainQuery.execute({ query: `SOP execution: ${sop.title} ${input}`, limit: 3, company });
