...
export async function assignAgencyToTask(projectId: string, taskId: string, agencyConfig: AgencyConfig, memory: EpisodicMemory): Promise<string> {
    ...
    let initialContext = agencyConfig.initial_context;

    if (!assignedAgencyId) {
        // LLM reasoning to refine the initial context based on project goals
        try {
            const llm = createLLM();
            const prompt = `You are an orchestrator spawning a child agency.
Task: ${task.description}
Role: ${agencyConfig.role}
Provided Context: ${agencyConfig.initial_context}
Refine and expand the context into a robust mission statement and context block for the new agent.`;
            // LLM class structure uses .generate()
            const reasoning = await llm.generate(prompt);
            if (reasoning && reasoning.trim().length > 0) {
                 initialContext = reasoning.trim();
            }
        } catch (e) {
            console.warn("LLM reasoning for spawning context failed, using default context:", e);
        }
