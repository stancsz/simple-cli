...
export async function evaluateDemandLogic(
    mcp: MCP,
    project_id?: string,
    client_id?: string,
    threshold_issues: number = 5
) {
    const logs: string[] = [];
    let issueCount = 0;
...
export async function scaleSwarmLogic(
    mcp: MCP,
    client_id: string,
...
export function registerScalingTools(server: McpServer, mcpClient?: MCP) {
    ...
    server.tool(
        ...
        async ({ project_id, client_id, threshold_issues }) => {
            try {
                const result = await evaluateDemandLogic(mcp, project_id, client_id, threshold_issues);
                return {
                    content: [
                        {
                            type: "text",
    ...
    server.tool(
        ...
        async ({ client_id, action, role, task, agent_id }) => {
            try {
                const result = await scaleSwarmLogic(mcp, client_id, action, role, task, agent_id);
                return {
                    content: [
                        {
                            type: "text",
