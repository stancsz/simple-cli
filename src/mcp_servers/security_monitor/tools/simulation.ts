    ...
    for (const vector of vectors) {
        try {
            if (vector === 'sqli') {
                const startTime = Date.now();
                try {
                    await axios.post(`${internalBase}/api/data`, { query: "'; DROP TABLE users; --" }, { validateStatus: () => true });
                } catch(e) {}
                syntheticLogs.push({ timestamp: new Date().toISOString(), endpoint: `${internalBase}/api/data`, status: 500, duration: Date.now() - startTime, payload: "'; DROP TABLE users; --" });

                try {
                    await axios.get(`${internalBase}/api/users?id=admin' OR '1'='1`, { validateStatus: () => true });
                } catch(e) {}
                syntheticLogs.push({ timestamp: new Date().toISOString(), endpoint: `${internalBase}/api/users`, status: 500, duration: Date.now() - startTime, payload: "admin' OR '1'='1" });
            } else if (vector === 'xss') {
                const startTime = Date.now();
                try {
                    await axios.post(`${internalBase}/api/comments`, { comment: "<script>alert(1)</script>" }, { validateStatus: () => true });
                } catch(e) {}
                syntheticLogs.push({ timestamp: new Date().toISOString(), endpoint: `${internalBase}/api/comments`, status: 400, duration: Date.now() - startTime, payload: "<script>alert(1)</script>" });
            } else if (vector === 'credential_stuffing') {
                for (let i = 0; i < 5; i++) {
                    const startTime = Date.now();
                    try {
                        await axios.post(`${internalBase}/api/login`, { username: `user${i}`, password: "password123" }, { validateStatus: () => true });
                    } catch(e) {}
                    syntheticLogs.push({ timestamp: new Date().toISOString(), endpoint: `${internalBase}/api/login`, status: 401, duration: Date.now() - startTime, payload: `user${i}:pass123` });
                }
            }
        } catch(e) {
