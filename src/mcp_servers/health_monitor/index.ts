...
function getAlertRulesFile() {
  return join(process.cwd(), '.agent', 'alert_rules.json');
}
...
export class HealthMonitorServer {
  ...
  private setupTools() {
    ...
    this.server.tool(
      "get_metrics",
      "Get raw metrics for a specific timeframe.",
      {
        timeframe: z.enum(["last_hour", "last_day", "last_week"]).describe("Timeframe for the metrics")
      },
      async ({ timeframe }) => {
        let days = 1;
        if (timeframe === "last_week") days = 7;
        ...
        return { content: [{ type: "text", text: JSON.stringify(allMetrics) }] };
      }
    );
    ...
    this.server.tool(
      ...
      async ({ metric, threshold, operator, contact }) => {
         const alertRulesFile = getAlertRulesFile();
         let rules: any[] = [];
  ...
  async run() {
    ...
    if (port) {
      ...
      app.listen(port, () => {
          console.log(`Health Monitor running on port ${port} (SSE)`);
      });
    } else {
