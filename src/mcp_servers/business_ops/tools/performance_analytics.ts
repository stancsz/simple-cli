...
export async function collectPerformanceMetrics(timeframe: string, clientId?: string): Promise<PerformanceMetrics> {
    ...
    else if (timeframe === "last_year") startDate.setFullYear(endDate.getFullYear() - 1); // Added handling for potential future cases or defaults
...
export function registerPerformanceAnalyticsTools(server: McpServer) {
    server.tool(
        ...
        async ({ timeframe, clientId }) => {
            const metrics = await collectPerformanceMetrics(timeframe, clientId);
