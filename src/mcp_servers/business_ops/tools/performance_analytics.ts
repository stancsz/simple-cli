...
export async function collectPerformanceMetrics(timeframe: "last_30_days" | "last_quarter" | "year_to_date" = "last_30_days", clientId?: string): Promise<PerformanceMetrics> {
...
export function registerPerformanceAnalyticsTools(server: McpServer) {
    server.tool(
        ...
        async ({ timeframe, clientId }) => {
            const metrics = await collectPerformanceMetrics(timeframe as any, clientId);
