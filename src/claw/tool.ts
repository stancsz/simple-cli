...
export const claw = {
  ...
  inputSchema: z.object({
    action: z
      .enum(["agent", "list_skills", "inspect_skill"])
      .describe("Action to perform"),
    message: z
      .string()
      .optional()
      .describe("Message for the agent (required for action=agent)"),
    skill_name: z.string().optional().describe("Name of the skill to inspect"),
    session_id: z
      .string()
      .optional()
      .describe("Custom session ID for the agent"),
    agent_id: z
      .string()
      .optional()
      .describe("Specific agent ID to use (e.g. openai-agent)"),
  }),
  execute: async ({
    action,
    message,
    ...
    try {
      ...
      if (action === "inspect_skill") {
        if (!skill_name)
          return "Error: skill_name is required for inspect_skill";
